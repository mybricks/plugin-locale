      ...
      return fetchPack({ link }).then(res => {
        ...
        try {
          if (data.enableFormat || defaultTransform) {
            let i18nLangContent = this.transform(res, data)
            if (typeof i18nLangContent !== 'object' || Object.values(i18nLangContent).some(item => !('id' in item || 'content' in item))) {
              data.errorMsg = '请返回指定的格式'
              return {}
              // throw new Error(`转化函数返回的必须是对象！并且每个语料都必须包含id和content字段`)
            }
            // 兼容旧的格式
            if (Array.isArray(i18nLangContent)) {
              i18nLangContent = i18nLangContent.reduce((res, item) => {
                res[item.id] = item
                return res
              }, {})
            }
